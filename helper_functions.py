...
# Funkcja do generowania przykładowych predykcji i ich wyświetlania
def display_test_predictions(model, dataset, class_names):
    ...
    for images, labels in dataset.take(1):  # Bierzemy jeden batch z datasetu
        ...
        for i in range(25):  # Wyświetlamy pierwsze 25 obrazów z tego batcha
            plt.subplot(5, 5, i + 1)
            plt.xticks([])
            plt.yticks([])
            plt.grid(False)
            plt.imshow(images[i], cmap=plt.cm.binary)
            plt.xlabel(f"Actual: {class_names[labels[i]]}\nPredicted: {class_names[predictions_classes[i]]}")
    plt.show()